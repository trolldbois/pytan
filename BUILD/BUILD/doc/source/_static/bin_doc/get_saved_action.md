...
```bash
bin/get_saved_action.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --port 443 --loglevel 1 --all --file "/tmp/out.json" --export_format json
```
...
Report file '/tmp/out.json' written with 6864 bytes
```
...
```bash
bin/get_saved_action.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --port 443 --loglevel 1 --all --file "/tmp/out.csv"
```

```
PyTan v2.1.4 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  SavedActionList, len: 4
Report file '/tmp/out.csv' written with 2214 bytes
```
...
```bash
bin/get_saved_action.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --port 443 --loglevel 1 --all --file "/tmp/out.xml"
```

```
PyTan v2.1.4 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  SavedActionList, len: 4
Report file '/tmp/out.xml' written with 2214 bytes
```
...
###### generated by: `build_bin_doc v2.1.0`, date: Mon Sep 14 15:36:09 2015 EDT, Contact info: **Jim Olsen <jim.olsen@tanium.com>**