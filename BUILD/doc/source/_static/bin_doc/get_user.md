...
```bash
bin/get_user.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --loglevel 1 --all --file "/tmp/out.json" --export_format json
```

```
PyTan v2.1.0 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  UserList, len: 14
Report file '/tmp/out.json' written with 22211 bytes
```
...
```bash
bin/get_user.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --loglevel 1 --all --file "/tmp/out.csv"
```

```
PyTan v2.1.0 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  UserList, len: 14
Report file '/tmp/out.csv' written with 8995 bytes
```
...
```bash
bin/get_user.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --loglevel 1 --all --file "/tmp/out.xml"
```

```
PyTan v2.1.0 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  UserList, len: 14
Report file '/tmp/out.xml' written with 8995 bytes
```
...
###### generated by: `build_bin_doc v2.1.0`, date: Thu Sep  3 22:03:43 2015 EDT, Contact info: **Jim Olsen <jim.olsen@tanium.com>**