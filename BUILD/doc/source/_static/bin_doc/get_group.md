...
usage: get_group.py [-h] [-u USERNAME] [-p PASSWORD] [--session_id SESSION_ID]
                    ...
                    [--all] [--id ID] [--name NAME] [--file REPORT_FILE]
...
```bash
bin/get_group.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --loglevel 1 --all --file "/tmp/out.json" --export_format json
```

```
PyTan v2.1.0 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  GroupList, len: 4
Report file '/tmp/out.json' written with 3338 bytes
```
...
```bash
bin/get_group.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --loglevel 1 --all --file "/tmp/out.csv"
```

```
PyTan v2.1.0 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  GroupList, len: 4
Report file '/tmp/out.csv' written with 1238 bytes
```
...
```bash
bin/get_group.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --loglevel 1 --all --file "/tmp/out.xml"
```

```
PyTan v2.1.0 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  GroupList, len: 4
Report file '/tmp/out.xml' written with 1238 bytes
```
...
###### generated by: `build_bin_doc v2.1.0`, date: Thu Sep  3 22:02:55 2015 EDT, Contact info: **Jim Olsen <jim.olsen@tanium.com>**