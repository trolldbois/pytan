...
<a name='toc'>Table of contents:</a>
  ...
  * [Help for Get Setting](#user-content-help-for-get-setting)
  * [Export all setting objects as JSON](#user-content-export-all-setting-objects-as-json)
  * [Export all setting objects as CSV](#user-content-export-all-setting-objects-as-csv)
  * [Export all setting objects as xml](#user-content-export-all-setting-objects-as-xml)

---------------------------

# Help for Get Setting

  * Print the help for get_setting.py
  * All scripts in bin/ will supply help if -h is on the command line
  * If passing in a parameter with a space or a special character, you need to surround it with quotes properly. On Windows this means double quotes. On Linux/Mac, this means single or double quotes, depending on what kind of character escaping you need.
  * If running this script on Linux or Mac, use the python scripts directly as the bin/get_setting.py
  * If running this script on Windows, use the batch script in the winbin/get_setting.bat so that python is called correctly.
...
```
usage: get_setting.py [-h] [-u USERNAME] [-p PASSWORD]
...
# Export all setting objects as JSON

  * Get all setting objects
  * Save the results to a JSON file

```bash
bin/get_setting.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --port 443 --loglevel 1 --all --file "/tmp/out.json" --export_format json
```

```
PyTan v2.1.5 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  SystemSettingList, len: 94
Report file '/tmp/out.json' written with 29090 bytes
```
...
# Export all setting objects as CSV

  * Get all setting objects
  * Save the results to a csv file

```bash
bin/get_setting.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --port 443 --loglevel 1 --all --file "/tmp/out.csv"
```

```
PyTan v2.1.5 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  SystemSettingList, len: 94
Report file '/tmp/out.csv' written with 6095 bytes
```
...
# Export all setting objects as xml

  * Get all setting objects
  * Save the results to a xml file

```bash
bin/get_setting.py -u Administrator -p 'Tanium2015!' --host 10.0.1.240 --port 443 --loglevel 1 --all --file "/tmp/out.xml"
```

```
PyTan v2.1.5 Handler for Session to 10.0.1.240:443, Authenticated: True, Platform Version: 6.5.314.4301
Found items:  SystemSettingList, len: 94
Report file '/tmp/out.xml' written with 6095 bytes
```
...
###### generated by: `build_bin_doc v2.1.0`, date: Tue Sep 15 18:25:42 2015 EDT, Contact info: **Jim Olsen <jim.olsen@tanium.com>**