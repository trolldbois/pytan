...
class BaseType(object):
    ...
    def toSOAPElement(self, minimal=False): # noqa
        ...
        for p in self._simple_properties:
            ...
            if val is not None or not minimal:
                root.append(el)
        for p, t in six.iteritems(self._complex_properties):
            val = getattr(self, p)
            # print p, t, val
            if val is not None or not minimal:
                ...
                if isinstance(val, BaseType):
                    ...
                    if val is not None:
                        el.append(str(val))
        for p, t in six.iteritems(self._list_properties):
            vals = getattr(self, p)
            # print p, t, vals
            if not vals:
                continue
    ...
    def fromSOAPElement(cls, el): # noqa
        result = cls()
        for p, t in six.iteritems(result._simple_properties):
            pel = el.find("./{}".format(p))
            if pel is not None and pel.text:
                setattr(result, p, t(pel.text))
            else:
                setattr(result, p, None)
        for p, t in six.iteritems(result._complex_properties):
            ...
            if len(elems) > 1:
                ...
                setattr(result, p, None)
        for p, t in six.iteritems(result._list_properties):
            setattr(result, p, [])
            elems = el.findall('./{}'.format(p))
            for elem in elems:
                if issubclass(t, BaseType):
    ...
    def to_flat_dict(self, prefix='', explode_json_string_values=False):
        ...
        for p, _ in six.iteritems(self._simple_properties):
            val = getattr(self, p)
            if val is not None:
                json_out = None
        ...
        for p, _ in six.iteritems(self._complex_properties):
            val = getattr(self, p)
            if val is not None:
        ...
        for p, _ in six.iteritems(self._list_properties):
            val = getattr(self, p)
            if val is not None:
