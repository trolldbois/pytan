...
examples = [
    {
        'name': 'Export user id 1 as JSON',
        'cmd': 'get_user.py $API_INFO --id 1 --file "$TMP/out.json" json',
        'notes': ['Get the first user object', 'Save the results to a JSON file'],
    ...
    {
        'name': 'Create a new user from the modified JSON file',
        'cmd': 'create_user_from_json.py $API_INFO -j "$TMP/out.json"',
...
if __name__ == "__main__":
    ...
    parser = utils.setup_create_json_object_argparser('user', __doc__)
