...
examples = [
    {
        'name': 'Export saved_question id 1 as JSON',
        'cmd': 'get_saved_question.py $API_INFO --id 1 --file "$TMP/out.json" json',
        'notes': ['Get the first saved_question object', 'Save the results to a JSON file'],
    ...
    {
        'name': 'Create a new saved_question from the modified JSON file',
        'cmd': 'create_saved_question_from_json.py $API_INFO -j "$TMP/out.json"',
...
if __name__ == "__main__":
    ...
    parser = utils.setup_create_json_object_argparser('saved_question', __doc__)
