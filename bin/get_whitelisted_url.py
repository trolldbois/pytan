...
examples = [
    {
        'name': 'Export all whitelisted_url as JSON',
        'cmd': 'get_whitelisted_url.py $API_INFO --all --file "$TMP/out.json" json',
        'notes': ['Get all whitelisted_url objects', 'Save the results to a JSON file'],
        'precleanup': 'rm -f $TMP/out.json',
        'file_exist': '$TMP/out.json',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all whitelisted_url as CSV',
        'cmd': 'get_whitelisted_url.py $API_INFO --all --file "$TMP/out.csv" csv',
        'notes': ['Get all whitelisted_url objects', 'Save the results to a csv file'],
        'precleanup': 'rm -f $TMP/out.csv',
        'file_exist': '$TMP/out.csv',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all whitelisted_url as xml',
        'cmd': 'get_whitelisted_url.py $API_INFO --all --file "$TMP/out.xml" xml',
        'notes': ['Get all whitelisted_url objects', 'Save the results to a xml file'],
