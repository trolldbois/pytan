...
examples = [
    {
        'name': 'Export all saved_question as JSON',
        'cmd': 'get_saved_question.py $API_INFO --all --file "$TMP/out.json" json',
        'notes': ['Get all saved_question objects', 'Save the results to a JSON file'],
        'precleanup': 'rm -f $TMP/out.json',
        'file_exist': '$TMP/out.json',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all saved_question as CSV',
        'cmd': 'get_saved_question.py $API_INFO --all --file "$TMP/out.csv" csv',
        'notes': ['Get all saved_question objects', 'Save the results to a csv file'],
        'precleanup': 'rm -f $TMP/out.csv',
        'file_exist': '$TMP/out.csv',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all saved_question as xml',
        'cmd': 'get_saved_question.py $API_INFO --all --file "$TMP/out.xml" xml',
        'notes': ['Get all saved_question objects', 'Save the results to a xml file'],
...
if __name__ == "__main__":
    ...
    parser = utils.setup_get_object_argparser('saved_question', __doc__)
