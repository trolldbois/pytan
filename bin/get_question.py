...
examples = [
    {
        'name': 'Export all question as JSON',
        'cmd': 'get_question.py $API_INFO --all --file "$TMP/out.json" json',
        'notes': ['Get all question objects', 'Save the results to a JSON file'],
        'precleanup': 'rm -f $TMP/out.json',
        'file_exist': '$TMP/out.json',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all question as CSV',
        'cmd': 'get_question.py $API_INFO --all --file "$TMP/out.csv" csv',
        'notes': ['Get all question objects', 'Save the results to a csv file'],
        'precleanup': 'rm -f $TMP/out.csv',
        'file_exist': '$TMP/out.csv',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all question as xml',
        'cmd': 'get_question.py $API_INFO --all --file "$TMP/out.xml" xml',
        'notes': ['Get all question objects', 'Save the results to a xml file'],
