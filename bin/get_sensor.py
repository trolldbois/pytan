...
examples = [
    {
        'name': 'Export all sensor as JSON',
        'cmd': 'get_sensor.py $API_INFO --all --file "$TMP/out.json" json',
        'notes': ['Get all sensor objects', 'Save the results to a JSON file'],
        'precleanup': 'rm -f $TMP/out.json',
        'file_exist': '$TMP/out.json',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all sensor as CSV',
        'cmd': 'get_sensor.py $API_INFO --all --file "$TMP/out.csv" csv',
        'notes': ['Get all sensor objects', 'Save the results to a csv file'],
        'precleanup': 'rm -f $TMP/out.csv',
        'file_exist': '$TMP/out.csv',
        'tests': 'exitcode, file_exist',
    },
    {
        'name': 'Export all sensor as xml',
        'cmd': 'get_sensor.py $API_INFO --all --file "$TMP/out.xml" xml',
        'notes': ['Get all sensor objects', 'Save the results to a xml file'],
